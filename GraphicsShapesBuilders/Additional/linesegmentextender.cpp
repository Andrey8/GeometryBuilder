...
void GSBuilders::Additional::LineSegmentExtender::CreateLine()
{
    ...
    QList< GraphicsPoint * > const points = GeomConstr::Objects::GetConstructionData().GetInteriorPoints( m_lineSegment );

    m_line = new GraphicsLine( p1, p2 );
    //p1->AddLineAsDefined( m_line );
    //p2->AddLineAsDefined( m_line );
    GeomConstr::Objects::GetConstructionData().AddLineAsDefined( p1, m_line );
    GeomConstr::Objects::GetConstructionData().AddLineAsDefined( p2, m_line );
    foreach ( GraphicsPoint * p, points )
    {
        //p->RemoveNondefinedLineSegment( m_lineSegment );
        GeomConstr::Objects::GetConstructionData().RemoveNondefinedLineSegment( p, m_lineSegment );

        m_line->PutPoint( p );
    }
    //p1->RemoveDefinedLineSegment( m_lineSegment );
    //p2->RemoveDefinedLineSegment( m_lineSegment );
    GeomConstr::Objects::GetConstructionData().RemoveDefinedLineSegment( p1, m_lineSegment );
    GeomConstr::Objects::GetConstructionData().RemoveDefinedLineSegment( p2, m_lineSegment );
